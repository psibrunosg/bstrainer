      ...
      <div className="grid grid-cols-2 gap-2">
        {options.map((o) => (
          <button key={o} onClick={() => onPick(o)}
            className={`rounded-2xl border px-4 py-4 text-sm font-semibold transition-all active:scale-[0.97] ${
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <div className="animate-rise">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-volt">Perfil de treino</p>
        <h1 className="font-display mt-1 text-3xl">Conta pra gente sobre você</h1>
      ...
      <div className="animate-rise" key={step}>
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="card-surface flex items-center justify-between rounded-2xl p-4">
              <button onClick={() => setDays((d) => Math.max(1, d - 1))} className="flex h-12 w-12 items-center justify-center rounded-xl border border-line text-xl text-muted-foreground active:scale-95">−</button>
              <span className="font-mono-num text-4xl font-bold text-volt">{days}x</span>
              <button onClick={() => setDays((d) => Math.min(7, d + 1))} className="flex h-12 w-12 items-center justify-center rounded-xl border border-line text-xl text-muted-foreground active:scale-95">+</button>
            </div>
              ...
              {EQUIPMENT.map((eq) => {
                ...
                return (
                  <button key={eq} onClick={() => setEquip((p) => (on ? p.filter((e) => e !== eq) : [...p, eq]))}
                    className={`flex items-center justify-between rounded-2xl border px-4 py-3.5 text-sm font-medium transition-all active:scale-[0.97] ${
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="space-y-1">
                ...
                <input value={weight} onChange={(e) => setWeight(e.target.value)} inputMode="decimal"
                  className="h-14 w-full rounded-2xl border border-line bg-surface px-4 font-mono-num text-lg outline-none focus:border-volt/60" />
              </label>
              <label className="space-y-1">
            ...
            <p className="card-surface rounded-2xl p-4 text-xs leading-relaxed text-muted-foreground">
      ...
      <div className="flex gap-2">
        {step > 0 && (
          <button onClick={() => setStep(step - 1)} className="flex h-13 items-center gap-2 rounded-2xl border border-line px-5 py-3.5 text-sm font-semibold text-muted-foreground">
            <ArrowLeft size={15} /> Voltar
          </button>
        )}
        <button onClick={() => (step < 3 ? setStep(step + 1) : onDone())}
          className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-volt py-3.5 font-display text-sm uppercase tracking-wide text-[#101405] active:scale-[0.98]">
